...
//        self.collectioView.register(
//            UINib(nibName: "HomeRankingContainerCell", bundle: nil),
...
//        )
        ...
        self.collectioView.register(UICollectionViewCell.self, forCellWithReuseIdentifier: "empty")
        self.collectioView.delegate = self
        self.collectioView.dataSource = self
        
        self.collectioView.collectionViewLayout = UICollectionViewCompositionalLayout.init(sectionProvider: { [weak self] section, _ in
    ...
    private func makeHeaderSection() -> NSCollectionLayoutSection {
        ...
        let group = NSCollectionLayoutGroup.vertical(
            layoutSize: layoutSize,
            subitems: [
                .init(layoutSize: .init(widthDimension: .absolute(0.1), heightDimension: .absolute(0.1))
                     )
            ]
        )
