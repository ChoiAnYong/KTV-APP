...
class ChattingView: UIView {
    ...
    @IBAction func closeDidTap(_ sender: Any) {
        self.textField.resignFirstResponder()//첫 번째 리스폰더의 자격을 잃도록 해줌(키보드 내리는범)
    ...
    private func scrollToLatestIfNeeded() {
        ...
        let isLastMessageMine = self.viewModel.message.last?.isMine == true
        
        if isBottomOffset || isLastMessageMine {
            self.collectionView.scrollToItem(
                at: IndexPath(item: self.viewModel.message.count - 1, section: 0),
                at: .bottom,
                animated: true)
        }
    }
...
extension ChattingView: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        let item = self.viewModel.message[indexPath.item]
